  ...
  return (
    <>
      ...
      <BtnRow>
        ...
        <Btn onClick={() => { if (!form.razao_social) return; onSave(form) }}>Salvar</Btn>
      </BtnRow>
    </>
  )
}
...
function ContratoForm({ initial = {}, onSave, onClose }) {
  const [form, setForm] = useState({
    ...
    ...initial,
    // Garantir que campos numéricos sejam números, não strings do banco
    horas_contratadas: +(initial.horas_contratadas ?? 0),
    km_contratados: +(initial.km_contratados ?? 0),
    // Garantir que datas sejam strings ISO sem hora
    inicio: initial.inicio ? initial.inicio.slice(0, 10) : '',
    termino: initial.termino ? initial.termino.slice(0, 10) : '',
  })
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }))
...
export default function ClienteDetail() {
  ...
  async function load() {
    const [{ data: cl }, { data: cts }] = await Promise.all([
      ...
      supabase.from('contratos').select('*').eq('cliente_id', id).neq('arquivado', true).order('created_at', { ascending: false })
    ])
    setCliente(cl)
    setContratos(cts || [])
  }
  ...
  async function saveCliente(form) {
    const { _action, id: _fid, created_at, updated_at, arquivado, contratos, ...payload } = form
    await supabase.from('clientes').update({ ...payload, updated_at: new Date() }).eq('id', cliente.id)
    setModal(null)
    load()
  }

  async function saveContrato(form) {
    const { _action, id: _formId, created_at, updated_at, arquivado, cliente_id: _cid, clientes, ...payload } = form
    if (modal?.id && modal?._action === 'edit') {
      await supabase.from('contratos').update({ ...payload, updated_at: new Date() }).eq('id', modal.id)
    } else {
      await supabase.from('contratos').insert({ ...payload, cliente_id: id })
    }
    setModal(null)
    load()
  }
  ...
  return (
    <Layout crumbs={[{ label: 'Clientes', to: '/clientes' }, { label: cliente.razao_social }]}>
        ...
        <PageHeader
          ...
          actions={
            <div style={{ display: 'flex', gap: 8 }}>
              <Btn variant="secondary" size="sm" onClick={() => navigate(`/clientes/${id}/dossie`)}>📋 Dossiê</Btn>
              <Btn variant="secondary" size="sm" onClick={e => { setModal({ ...cliente, _action: 'editCliente' }) }}>Editar</Btn>
      ...
      {modal?._action === 'editCliente' && (
        <Modal title="Editar Cliente" onClose={() => setModal(null)} width={560}>
          <ClienteForm initial={modal} onSave={saveCliente} onClose={() => setModal(null)} />
        </Modal>
      )}
