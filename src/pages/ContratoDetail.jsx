...
function VisitaForm({ initial = {}, onSave, onClose }) {
  const [form, setForm] = useState({
    data: '', os: '', tipo: 'preventiva', horas: '', km: '', obs: '',
    ...initial,
...
export default function ContratoDetail() {
  ...
  async function load() {
    ...
    const [{ data: eqs }, { data: vs }] = await Promise.all([
      supabase.from('equipamentos').select('*').eq('contrato_id', id).neq('arquivado', true).order('modelo'),
  ...
  async function saveEquip(form) {
    const { _action, id: _formId, created_at, updated_at, arquivado, contrato_id: _ctid, contratos, foto_url, ...payload } = form
    // Converter ano para inteiro ou null — banco não aceita string vazia
    if (payload.ano !== undefined) {
      payload.ano = payload.ano !== '' && payload.ano !== null ? parseInt(payload.ano, 10) : null
    }
    if (modal?.id && modal?._action === 'edit') {
      await supabase.from('equipamentos').update({ ...payload, updated_at: new Date() }).eq('id', modal.id)
    } else {
      await supabase.from('equipamentos').insert({ ...payload, contrato_id: id })
    }
    setModal(null)
    load()
  }
  ...
  async function saveVisita(form) {
    const { _action, id: _fid, created_at, contrato_id: _ctid, ...payload } = form
    if (modal?._action === 'editVisita') {
      await supabase.from('visitas').update({ ...payload }).eq('id', modal.id)
    } else {
      await supabase.from('visitas').insert({ ...payload, contrato_id: id })
    }
    setModal(null)
    load()
  }
  ...
  return (
    <Layout crumbs={[
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '28px 28px' }}>
        ...
        {contrato.tipo === 'manutencao' && visitas.length > 0 && (
          <Section title={`Visitas registradas (${visitas.length})`}>
            {visitas.map(v => (
              <div key={v.id} style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '10px 14px', marginBottom: 6, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: v.tipo === 'corretiva' ? 'var(--yellow-text)' : 'var(--blue)', flexShrink: 0 }} />
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 13, fontWeight: 500 }}>OS {v.os}</span>
                      <span style={{ fontSize: 11, padding: '1px 7px', borderRadius: 20, background: v.tipo === 'corretiva' ? 'var(--yellow-bg)' : 'var(--blue-bg)', color: v.tipo === 'corretiva' ? 'var(--yellow-text)' : 'var(--blue-text)' }}>
                        {v.tipo === 'corretiva' ? 'Corretiva' : 'Preventiva'}
                      </span>
                    </div>
                    <div style={{ fontSize: 11, color: 'var(--text-2)', marginTop: 1 }}>
                      {fmtDate(v.data)} · {v.horas}h · {v.km}km {v.obs ? `· ${v.obs}` : ''}
                    </div>
                  </div>
                </div>
                <button
                  onClick={() => setModal({ ...v, _action: 'editVisita' })}
                  style={{ background: 'none', border: '1px solid var(--border-2)', borderRadius: 6, color: 'var(--text-2)', padding: '3px 8px', fontSize: 11, cursor: 'pointer' }}>
                  Editar
                </button>
              </div>
            ))}
      ...
      {(modal === 'visita' || modal?._action === 'editVisita') && (
        <Modal title={modal === 'visita' ? 'Registrar Visita' : 'Editar Visita'} onClose={() => setModal(null)} width={520}>
          <VisitaForm initial={modal?._action === 'editVisita' ? modal : {}} onSave={saveVisita} onClose={() => setModal(null)} />
        </Modal>
      )}
