...
export default function EquipamentoDetail() {
  ...
  async function savePeca(form) {
    const { _action, id: formId, created_at, updated_at, equipamento_id, ...payload } = form
  ...
  async function saveBaixa(form) {
    ...
    const { data: novaBaixa } = await supabase.from('baixas').insert({ ...form, peca_id: peca.id }).select().single()

    // Busca visita vinculada à OS para incluir no PDF
    const { data: visitaDoc } = await supabase
      .from('visitas')
      .select('*')
    ...
    gerarPDFVisita({
      ...
      contrato,
      equipamento: equip,
      visita: visitaDoc || null,
      baixas: [{ ...form, peca_id: peca.id }],
      pecas,
