...
import { gerarPDFVisita, gerarPDFPreVisita } from '../lib/pdf'

// ── Formulário de peça ────────────────────────────────────────────────────────
function PecaForm({ initial = {}, tipo, onSave, onClose }) {
  const [form, setForm] = useState({
    descricao: '', codigo: '', unidade: 'PC', tipo: '', obs: '',
    ...initial,
    qtd_contratada: +(initial.qtd_contratada ?? 0),
  })
...
function BaixaForm({ peca, visitas, initial = {}, onSave, onClose }) {
  const [form, setForm] = useState({
    visita_id: '', qtd: '', re_nf: '',
    ...initial,
    qtd: initial.qtd ?? '',
...
// ── Modal pré-visita ──────────────────────────────────────────────────────────
function PreVisitaModal({ pecas, onGerar, onClose }) {
  const hoje = new Date().toISOString().slice(0, 10)
  const [dataEntrega, setDataEntrega] = useState(hoje)
  const [itens, setItens] = useState(
    pecas.map(p => ({ ...p, qtd: 1, selecionado: true }))
  )

  const setQtd = (id, qtd) => setItens(its => its.map(i => i.id === id ? { ...i, qtd: +qtd } : i))
  const toggleSel = (id) => setItens(its => its.map(i => i.id === id ? { ...i, selecionado: !i.selecionado } : i))

  const selecionados = itens.filter(i => i.selecionado && i.qtd > 0)

  return (
    <Modal title="Comprovante de Entrega — Pré-visita" onClose={onClose} width={520}>
      <p style={{ fontSize: 12, color: 'var(--text-2)', marginBottom: 14 }}>
        Selecione os materiais a serem entregues. O número da OS ficará em branco para preenchimento posterior.
      </p>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-2)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '.06em' }}>
          Materiais
        </div>
        <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {['', 'Descrição', 'Código', 'Unid.', 'Qtd. Prevista'].map(h => (
                  <th key={h} style={{ background: 'var(--bg-3)', color: 'var(--text-2)', fontSize: 10, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase', padding: '7px 10px', textAlign: h === 'Qtd. Prevista' ? 'center' : 'left', borderBottom: '1px solid var(--border)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {itens.map((item, i) => (
                <tr key={item.id} style={{ background: item.selecionado ? '' : 'var(--bg-3)', opacity: item.selecionado ? 1 : 0.5 }}>
                  <td style={{ padding: '8px 10px', borderBottom: i < itens.length - 1 ? '1px solid var(--border)' : 'none' }}>
                    <input type="checkbox" checked={item.selecionado} onChange={() => toggleSel(item.id)} style={{ width: 'auto', cursor: 'pointer' }} />
                  </td>
                  <td style={{ padding: '8px 10px', fontSize: 12, borderBottom: i < itens.length - 1 ? '1px solid var(--border)' : 'none' }}>{item.descricao}</td>
                  <td style={{ padding: '8px 10px', fontSize: 11, fontFamily: 'monospace', color: 'var(--text-2)', borderBottom: i < itens.length - 1 ? '1px solid var(--border)' : 'none' }}>{item.codigo}</td>
                  <td style={{ padding: '8px 10px', fontSize: 11, color: 'var(--text-2)', borderBottom: i < itens.length - 1 ? '1px solid var(--border)' : 'none' }}>{item.unidade}</td>
                  <td style={{ padding: '6px 10px', borderBottom: i < itens.length - 1 ? '1px solid var(--border)' : 'none' }}>
                    <input
        ...
        {selecionados.length === 0 && (
          <p style={{ fontSize: 11, color: 'var(--red-text)', marginTop: 6 }}>Selecione ao menos um material.</p>
        )}
      </div>
      ...
      <BtnRow>
        ...
        <Btn
          ...
          onClick={() => onGerar({ dataEntrega, itens: selecionados })}
        >
          Gerar Comprovante
        </Btn>
      </BtnRow>
...
export default function EquipamentoDetail() {
  ...
  const [showPreVisita, setShowPreVisita] = useState(false)
  ...
  async function saveBaixa(form) {
    ...
    if (modal?._action === 'editBaixa') {
      const { _action, id: _bid, peca_id: _pid, created_at, ...payload } = form
      await supabase.from('baixas').update(payload).eq('id', modal.id)
    } else {
  ...
  async function excluirBaixa(bid) {
    await supabase.from('baixas').delete().eq('id', bid)
    setModal(null)
    load()
  }

  function gerarPreVisitaPDF({ dataEntrega, itens }) {
    gerarPDFPreVisita({
      cliente,
      contrato,
      equipamento: equip,
  ...
  return (
    <Layout crumbs={[
        ...
        >
                                  ...
                                  <tbody>
                                    {bxs.map((b, bi) => (
                                      <tr key={b.id}>
                                        ...
                                        <td style={{ padding: '6px 10px', borderBottom: bi < bxs.length - 1 ? '1px solid var(--border)' : 'none' }}>
                                          <div style={{ display: 'flex', gap: 4 }}>
                                            <button
                                              onClick={() => setModal({ peca: p, ...b, _action: 'editBaixa' })}
                                              style={{ fontSize: 10, padding: '2px 7px', borderRadius: 5, border: '1px solid var(--border-2)', background: 'transparent', color: 'var(--text-2)', cursor: 'pointer' }}>
                                              ✎
                                            </button>
                                            <button
                                              onClick={() => setModal({ ...b, _action: 'deleteBaixa' })}
                                              style={{ fontSize: 10, padding: '2px 7px', borderRadius: 5, border: '1px solid var(--red-bg)', background: 'transparent', color: 'var(--red-text)', cursor: 'pointer' }}>
                                              ✕
                                            </button>
                                          </div>
                                        </td>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {pecas.length > 0 && (
                <button
                  onClick={() => setShowPreVisita(true)}
                  style={{ fontSize: 12, padding: '6px 14px', borderRadius: 'var(--radius)', border: '1px solid var(--orange)', background: 'var(--orange-bg)', color: 'var(--orange-text)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
                  📋 Comprovante pré-visita
                </button>
              )}
      ...
      {/* Baixa de peça (nova ou edição) */}
      {(modal?._action === 'baixa' || modal?._action === 'editBaixa') && (
        <Modal title={modal._action === 'editBaixa' ? 'Editar Baixa' : 'Registrar Baixa'} onClose={() => setModal(null)} width={440}>
          <BaixaForm
            peca={modal.peca}
            visitas={visitas}
            initial={modal._action === 'editBaixa' ? modal : {}}
            onSave={saveBaixa}
      ...
      {/* Excluir baixa */}
      {modal?._action === 'deleteBaixa' && (
        <Confirm
          danger
          message={`Excluir esta baixa de ${modal.qtd} unidade(s)? O saldo da peça será restaurado.`}
          onConfirm={() => excluirBaixa(modal.id)}
      ...
      {showPreVisita && (
        <PreVisitaModal
          pecas={pecas}
          onGerar={gerarPreVisitaPDF}
          onClose={() => setShowPreVisita(false)}
        />
      )}

      {/* Gerar PDF pós-visita */}
