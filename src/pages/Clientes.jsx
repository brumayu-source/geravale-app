...
export default function Clientes() {
  ...
  async function load() {
    ...
    const [{ data: cls }, { data: cts }] = await Promise.all([
      supabase.from('clientes').select('*').neq('arquivado', true).order('razao_social'),
      supabase.from('contratos').select('id, cliente_id, tipo').neq('arquivado', true)
    ])
    setClientes(cls || [])
    setContratos(cts || [])
    setLoading(false)
  ...
  async function saveCliente(form) {
    const { _action, id: _fid, created_at, updated_at, arquivado, ...payload } = form
    const isEdit = modal?.id && modal?._action === 'edit'
    if (isEdit) {
